...
//   }
    ...
    for (var j = 0; j < modelTypesIds.length; j++) {
        ...
        for (var i = 0; i < items.length; i++)
        {
            ...
            var item = items[i];
            if(item.location.search(j)>=0){
               
                // modelTypesIds.search(modelTypesNum);
            var itemdivinner = $("#" + modelTypesIds[modelTypesNum.indexOf(item.location)] + "-wrapper");
           
            var modelformat = (item.format) ? ' model-format="' + item.format + '"' : "";

            // alert(modelTypesNum[i]);

            var itemhtml = '<div class="col-sm-6"><div class="float-left-eight-component">' + '<a class="thumbnail add-item items"' + ' model-meta=\'' + JSON.stringify(item.meta || {}) + '\' model-name="' + item.name + '"' + ' model-url="' + item.model + '"' + ' model-location="' + item.location + '"' + modelformat + '>' + '<img src="' + item.image + '" alt="Add Item" data-dismiss="modal"><span class="items-title"> ' + item.name + '</span></a></div></div>'
    ...
    for (var j = 0; j < modelTypesclasses.length; j++) {
        ...
        $(".left-sidebar-items").append(html);
            ...
            var itemdivinner = $("#" + modelTypesclasses[imageTypesNum.indexOf(item.location)] + "-wrapper");
            //$("#floor_items-wrapper")
            var modelformat = (item.format) ? ' model-format="' + item.format + '"' : "";
            // alert(modelTypesNum[i]);
            var itemhtml = '<div class="c3-item-boxes items-two panel-button-one">' + '<a class="thumbnail c3-items"' + ' model-meta=\'' + JSON.stringify(item.meta || {}) + '\' model-name="' + item.name + '"' + ' model-url="' + item.model + '"' + ' model-location="' + item.location + '"' + modelformat + '>' + '<img src="' + item.image + '" alt="Add Item" data-dismiss="modal" class="left-first-box hovereffect left-box-icons"><span class="items-title c-one--titles"> ' + item.name + '</span></a></div>'
            itemdivinner.append(itemhtml);
        }
        //html += '</div>';
